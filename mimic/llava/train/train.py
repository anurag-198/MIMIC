...
from llava.train.custom_attention import replace_qwen_attn
...
class ModelArguments:
    ...
    layers: Optional[str] = field(default='12,23')
...
class LazySupervisedDataset(Dataset):
        ...
        if "{" in data_path and "}" in data_path:
            ...
            with open(data_path, "r") as file:
                ...
                for dataset in datasets:
                    ...
                    new_data_dict = []
                    for it in cur_data_dict:
                        if len(it['image']) < 11:
                            new_data_dict.append(it)
                    if len(new_data_dict) != len(cur_data_dict):
                        print('new len and old len ', len(new_data_dict), len(cur_data_dict))

                    cur_data_dict = new_data_dict
                    print('new len is ', len(cur_data_dict))
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            img_cnt = sum(len(conv["value"].split('<image>')) for conv in sample["conversations"]) - 2 
            assert cur_len > 0, f"Conversation length is 0 for {sample}"
            if "image" in sample or "video" in sample or self.data_args.early_mix_text: # each image has 729 tokens (taken that into account)
                length_list.append(cur_len + 729*img_cnt)
            else:
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        #print('doing for ................ ', len(sources['image']))
        # if 'id' in sources:
        #     print('id is ', sources['id'])
        # elif 'sample_id' in sources:
        #     print('id is ', sources['sample_id'])
        # else:
        #     print('no id present ')
        ...
        if "image" in sources[0]:
            ...
            if type(image_file) is list:
                ### update: count <image> in conversations and match it conversation, if not matching, add the image initially ###
                img_cnt = len(image_file)
                img_cnt_conv = sum([len(re.findall(DEFAULT_IMAGE_TOKEN, sentence["value"])) for sentence in sources[0]['conversations']])
                if img_cnt_conv != img_cnt:
                    print('sources ------ ', sources)
                if img_cnt_conv < img_cnt: ## annotation forgot to put <img> tags 
                    remaining_ = img_cnt - img_cnt_conv
                    sources[0]['conversations'][0]['value'] = DEFAULT_IMAGE_TOKEN * remaining_ + '\n' + sources[0]['conversations'][0]['value']
                ...
                img_cnt_conv = sum([len(re.findall(DEFAULT_IMAGE_TOKEN, sentence["value"])) for sentence in sources[0]['conversations']])
                if img_cnt_conv == 0: ## not sure what to do with more <image> tags > 1, but if 0, place one
            ...
            try:
                if "train_300k" in video_file: ### updated from shareGPT as it uses the same 
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    # if model_args.freeze_projector is not None:
    #     overwrite_config["freeze_projector"] = model_args.freeze_projector
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            elif 'qwen2-1.5b' in model_args.model_name_or_path.lower() or '1_5b' in model_args.model_name_or_path.lower():
                llava_cfg = AutoConfig.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
                customized_kwargs.pop('config')
...
class DetailedLRCallback(TrainerCallback):
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if state.global_step % 2 == 0:
            ...
            if lr_scheduler:
                print(f"Scheduler type: {type(lr_scheduler).__name__}")
...
def train(attn_implementation=None):
    ...
    ############### setting layers for masking ########################
    os.environ["LAYERS"] = str(model_args.layers)

    bnb_model_from_pretrained_args = {}
    
    if training_args.mask_enable:
        replace_qwen_attn()
    ...
    if model_args.vision_tower is not None:
        ...
        if data_args.image_grid_pinpoints is not None:
            if isinstance(data_args.image_grid_pinpoints, str) and "x" in data_args.image_grid_pinpoints:
                ...
                #print('here ------------------ ', matches, data_args.image_grid_pinpoints)
    ...
    #trainer = LLaVATrainer(model=model, tokenizer=tokenizer, args=training_args, callbacks=[DetailedLRCallback()], **data_module)
