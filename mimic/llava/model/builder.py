    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                elif 'qwen2-1.5b' in model_path or '1_5b' in model_path.lower():
                    if overwrite_config is not None:
                        llava_cfg = AutoConfig.from_pretrained(model_path,trust_remote_code=True)
                        
                        lora_path = overwrite_config.pop('lora_path', None)
                        
                        rank0_print(f"Overwriting config with {overwrite_config}")
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
                        
                        model = AutoModelForCausalLM.from_pretrained(model_path, trust_remote_code=True, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                        
                        if not os.path.exists(os.path.join(lora_path, 'adapter_config.json')):
                            shutil.copy(Path(lora_path).parent / 'adapter_config.json', os.path.join(lora_path, 'adapter_config.json'))
